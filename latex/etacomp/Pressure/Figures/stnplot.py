...
#a.text(lon+0.1,lat-0.05,0,50,"UNC Asheville")
#a.color(green)
...
a.color(80,80,240)
a.windbarb(-77.0,31.0,10,0,ccrad)	#Note how this is NOT from the north!
a.color(green)
...
a.sector(-78.0,31.5,10,15,0,135,'F') #x,y,inner_radius,outer_radius,angle1,angle2,[fill]
a.sector(-78.0,27.0,10,15,20,20,'F') #x,y,inner_radius,outer_radius,angle1,angle2,[fill]
a.sector(-78.0,24.0,10,15,135,0,'F') #x,y,inner_radius,outer_radius,angle1,angle2,[fill]
...
# and time.
	...
	a.color(50,150,255)
	if cloud == "CLR":
		a.circle(lon,lat,2) #Open circle
	if cloud == "FEW":
		a.circle(lon,lat,2,"F") #Open circle
	if cloud == "SCT":
	#	a.circle(clip_path=r'url(#marginmask)')
		a.circle(lon,lat,2,180,180,"F") #Open circle
	if cloud == "BKN":
		a.circle(lon,lat,2) #Open circle
	if cloud == "OVC":
		a.circle(lon,lat,2) #Open circle
	print (wspd, wdir)
	a.color(black)
	a.windbarb(lon,lat,wspd,wdir,20)		
 	if temp!=-9999:
		a.color(red)
		a.text(lon-.25,lat+.05,0,8,temp)
	if dwpt!=-9999:
		a.color(green)
		a.text(lon-.25,lat-.03,0,8,dwpt)
	if pres!=-9999:
		a.color(255,128,0)
		if pres>1000.0 and pres<1010.0:
			pres=str(int(round((pres-1000)*10)))
			a.text(lon+.05,lat+.05,0,8,"0"+pres)
		elif pres>1010.0:
			pres=str(int(round((pres-1000)*10)))
			a.text(lon+.05,lat+.05,0,8,pres)
		elif pres<1000.0:
			pres=str(int(round((pres-900)*10)))
			a.text(lon+.05,lat+.05,0,8,pres)
	a.color()
	a.linewidth()
	a.rect(-75.5,32.50,-79.0,48)
	a.text(48,-79,0,18,'METAR Observations')
	a.text(-79.0,48.0,0,18,'1900 UTC 15 February 2009')
